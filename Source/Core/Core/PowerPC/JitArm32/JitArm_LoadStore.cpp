...
void JitArm::SafeStoreFromReg(s32 dest, u32 value, s32 regOffset, int accessSize, s32 offset)
{
	...
	if (is_immediate)
	{
		...
		{
			// Move the value to be written into R0
			MOV(R0, RS);

			// Load the address of the helper function
			MOVI2R(R12, reinterpret_cast<u32>(&WriteToFifo));

			// Call the helper
			BLX(R12);

			// Track how many bytes were written (optional, for JIT stats)
			this->js.fifoBytesSinceCheck += accessSize >> 3;
...
void JitArm::WriteToFifo(u32 data)
{
  Fifo::PushFifoAuxBuffer(&data, sizeof(data));
}
