...
void ARMXEmitter::BLX(ARMReg reg)
{
  // Encoding for BLX <Rm> in ARM mode: 0x012FFF3X
  // where X = register number
  Write32(0x012FFF30 | reg);
}
...
void ARMXEmitter::B(CCFlags cond, const void* target)
{
  s32 offset = static_cast<const u8*>(target) - reinterpret_cast<const u8*>(GetCodePtr());
  offset >>= 2;  // ARM branch offset is in words

  ASSERT_MSG(DYNA_REC, offset >= -0x00800000 && offset <= 0x007FFFFF,
             "Branch target out of range: {} -> {}", fmt::ptr(GetCodePtr()), fmt::ptr(target));

  u32 instruction = 0x0A000000 | (cond << 28) | (offset & 0x00FFFFFF);
  Write32(instruction);
}

void ARMXEmitter::HINT(u32 opcode)
{
  // ARMv7 hint instructions are encoded as:
  // 0xE320F000 | (opcode & 0xF)
  // where opcode:
  //   0x0 = NOP
  //   0x1 = YIELD
  //   0x2 = WFE
...
void ARMXEmitter::WriteStoreOp(u32 Op, ARMReg Rt, ARMReg Rn, Operand2 Rm, bool RegAdd)
{
	...
	if (op == -1)
		ASSERT_MSG(DYNA_REC, false, "{} does not support {}", LoadStoreNames[Op], static_cast<int>(Rm.GetType()));
...
{
	ASSERT_MSG(DYNA_REC, Src >= S0 && Src <= D31, "Passed invalid src register to VSTR");
	ASSERT_MSG(DYNA_REC, Base <= R15, "Passed invalid base register to VSTR");

	bool Add = offset >= 0 ? true : false;
	u32 imm = abs(offset);

	ASSERT_MSG(DYNA_REC, (imm & 0xC03) == 0, "VSTR: Offset needs to be word aligned and small enough");

	if (imm & 0xC03)
		ERROR_LOG_FMT(DYNA_REC, "VSTR: Bad offset {:08x}", imm);
...
{
	ASSERT_MSG(DYNA_REC, Src < S0, "This VMOV doesn't support SRC other than ARM Reg");
	ASSERT_MSG(DYNA_REC, Dest >= D0, "This VMOV doesn't support DEST other than VFP");
