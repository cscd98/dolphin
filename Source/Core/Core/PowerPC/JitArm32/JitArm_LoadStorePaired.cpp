...
void JitArm::psq_l(UGeckoInstruction inst)
{
	...
	LDR(R11, R9, PPCSTATE_RUNTIME_OFFSET(spr[SPR_GQR0 + inst.I]));
	...
	MOVI2R(R14, (u32)asm_routines.paired_load_quantized);
	ADD(R14, R14, R12);
...
void JitArm::psq_lx(UGeckoInstruction inst)
{
	...
	LDR(R11, R9, PPCSTATE_RUNTIME_OFFSET(spr[SPR_GQR0 + inst.Ix]));
	UBFX(R12, R11, 16, 3); // Type
	LSL(R12, R12, 2);
	UBFX(R11, R11, 24, 6); // Scale
	LSL(R11, R11, 3);
...
void JitArm::psq_st(UGeckoInstruction inst)
{
	...
	if (update)
		MOV(gpr.R(inst.RA), R10);
	MOVI2R(R14, (u32)asm_routines.paired_store_quantized);
	ADD(R14, R14, R12);
	LDR(R14, R14, inst.W ? 8 * 4 : 0);
