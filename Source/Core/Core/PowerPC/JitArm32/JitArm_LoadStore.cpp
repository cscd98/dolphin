...
{
  JIT_LOG_NUM("SafeStoreFromReg: dest", dest);
  JIT_LOG_NUM("SafeStoreFromReg: value", value);
  JIT_LOG_NUM("SafeStoreFromReg: regOffset", regOffset);
  JIT_LOG_NUM("SafeStoreFromReg: accessSize", accessSize);
  JIT_LOG_NUM("SafeStoreFromReg: offset", offset);

  // ============================================================================
  // CRITICAL: Lock R10/R11/R12 Before Any Register Allocation
  // ============================================================================
  // EmitBackpatchRoutine() hardcodes these registers as scratch:
  //   - R10: temp2 (used in fast access path)
  //   - R11: temp (used in both fast and slow paths)
  //   - R12: addr (conditional, used when RS != R12)
  // Additionally, gather-pipe code path uses:
  //   - R10: GPFifo address calculations (lines 152-198)
  //   - R11: GPFifo pointer operations
  //   - R12: GPFifo base pointer
  // Without this lock, register cache can allocate these to PPC registers,
  // causing corruption when backpatch/gather-pipe code overwrites them.
  gpr.Lock(R10, R11, R12);
  ...
  // ============================================================================
  // CRITICAL: Unlock R10/R11/R12 After All Usage Complete
  // ============================================================================
  gpr.Unlock(R10, R11, R12);
}
...
{
  JIT_LOG_REG("SafeLoadToReg dest", dest);
  JIT_LOG_NUM("SafeLoadToReg addr", addr);
  JIT_LOG_NUM("SafeLoadToReg offsetReg", offsetReg);
  JIT_LOG_NUM("SafeLoadToReg accessSize", accessSize);
  JIT_LOG_NUM("SafeLoadToReg offset", offset);
  JIT_LOG_NUM("SafeLoadToReg update", update);

  // ============================================================================
  // CRITICAL: Lock R10/R11/R12 Before Any Register Allocation
  // ============================================================================
  // EmitBackpatchRoutine() hardcodes these registers as scratch:
  //   - R10: temp2 (used in fast access path for FP loads/stores)
  //   - R11: temp (used in both fast and slow paths)
  //   - R12: addr (conditional, used when dest != R12)
  // Without this lock, register cache can allocate these to PPC registers,
  // causing corruption when backpatch code overwrites them.
  gpr.Lock(R10, R11, R12);

  if (addr != -1 && !gpr.IsImm(addr))
  {
    JIT_LOG_NUM("SafeLoadToReg addr value", gpr.GetImm(addr));
    gpr.BindToRegister(addr, true);
  }

  if (offsetReg != -1 && !gpr.IsImm(offsetReg))
  {
    JIT_LOG_NUM("SafeLoadToReg offsetReg value", gpr.GetImm(offsetReg));
  ...
  // host ARM register bound to the PPC GPR addr
  ARMReg addr_reg = (addr != -1 && !gpr.IsImm(addr)) ? gpr.R(addr) : INVALID_REG;
  JIT_LOG_REG("Bound RA host reg", addr_reg);

  {
    ARMReg tmp = gpr.GetScopedReg();
    LDR(tmp, PPC_REG, PPCSTATE_OFF_GPR(addr));
    JIT_LOG_REG("Runtime RA value", tmp);
  }
	...
	else
	{
		if (addr != -1)
		{
			...
			{
				ADD(rA, addr_reg, gpr.R(offsetReg));
			}
		}
		else
		{
...
void JitArm::lXX(UGeckoInstruction inst)
{
  ...
  /*{
    ...
    LDR(tmp, PPC_REG, PPCSTATE_OFF_SPR(SPR_LR));
	  JIT_LOG_REG("lXX: LR at start is:", tmp);
  }*/
  ...
  // Log the fact that we skipped the SafeLoadToReg.
  {
    ...
    //JIT_LOG_NUM("Skipped SafeLoadToReg due to exception, Exceptions=", tmp);
  }
}
