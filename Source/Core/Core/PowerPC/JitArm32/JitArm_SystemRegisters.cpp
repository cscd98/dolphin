...
FixupBranch JitArm::JumpIfCRFieldBit(int field, int bit, bool jump_if_set)
{
	...
	case PowerPC::CR_SO_BIT:  // check bit 61 set
		LDR(RA, R9, PPCSTATE_RUNTIME_OFFSET(cr.fields[field]) + sizeof(u32));
		TST(RA, SOBit);
		branch = B_CC(jump_if_set ? CC_NEQ : CC_EQ);
	break;
	case PowerPC::CR_EQ_BIT:  // check bits 31-0 == 0
		LDR(RA, R9, PPCSTATE_RUNTIME_OFFSET(cr.fields[field]));
		CMP(RA, 0);
...
void JitArm::mtsr(UGeckoInstruction inst)
{
	...
	STR(gpr.R(inst.RS), R9, PPCSTATE_RUNTIME_OFFSET(sr[inst.SR]));
}
...
void JitArm::mfsr(UGeckoInstruction inst)
{
	...
	gpr.BindToRegister(inst.RD, false);
	LDR(gpr.R(inst.RD), R9, PPCSTATE_RUNTIME_OFFSET(sr[inst.SR]));
}
...
void JitArm::mcrf(UGeckoInstruction inst)
{
	...
	{
		LDR(rA, R9, PPCSTATE_RUNTIME_OFFSET(cr.fields[inst.CRFS]));
		STR(rA, R9, PPCSTATE_RUNTIME_OFFSET(cr.fields[inst.CRFD]));
		LDR(rA, R9, PPCSTATE_RUNTIME_OFFSET(cr.fields[inst.CRFS]) + sizeof(u32));
		STR(rA, R9, PPCSTATE_RUNTIME_OFFSET(cr.fields[inst.CRFD]) + sizeof(u32));
	}
	gpr.Unlock(rA);
}
