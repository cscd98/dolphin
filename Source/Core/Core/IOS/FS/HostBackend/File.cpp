...
Result<u32> HostFileSystem::ReadBytesFromFile(Fd fd, u8* ptr, u32 count)
{
  ...
  {
    const size_t actually_read_vfs = Libretro::File::ReadBytes(handle->host_file->GetVFSHandle(), ptr, count);

    if (actually_read_vfs != count)
      return ResultCode::AccessDenied;

    handle->file_offset += static_cast<u32>(actually_read_vfs);
    return static_cast<u32>(actually_read_vfs);
  }
#endif
