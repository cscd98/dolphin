...
void Init()
{
  is_enabled = Libretro::GetOption<bool>(Options::main_interface::ENABLE_LIBRETRO_VFS, /*def=*/true);
  ...
  if (!vfs_interface)
  {
    WARN_LOG_FMT(BOOT, "LibRetro VFS V3 not available, using standard file I/O");
  }
}
...
{
  int32_t size = -1;
  int flags = vfs_interface->stat(path.c_str(), &size);
              ...
              const char* path)
{
  ...
  int32_t tmp_size = 0;
  int flags = vfs_interface->stat(path, &tmp_size);
...
u64 GetSize(const std::string& path)
{
  ...
  if (vfs_interface->stat(path.c_str(), &size) == 0)
  {
...
::File::FSTEntry ScanDirectoryTree(const std::string& directory, bool recursive)
{
  ...
  if (recursive)
  {
    std::stack<::File::FSTEntry*> dir_stack;
    dir_stack.push(&parent_entry);

    while (!dir_stack.empty())
    {
      ::File::FSTEntry* current = dir_stack.top();
      dir_stack.pop();

      DEBUG_LOG_FMT(COMMON, "ScanDirectoryTree (recursive) {}", current->physicalName.c_str());

      retro_vfs_dir_handle* d = vfs_interface->opendir(current->physicalName.c_str(), false);
      if (!d)
        continue;
      ...
      while (vfs_interface->readdir(d))
      {
        ...
        entry.physicalName = current->physicalName + "/" + name;
        entry.isDirectory = vfs_interface->dirent_is_dir(d);
        entry.size = 0;

        current->children.push_back(entry);

        if (entry.isDirectory)
          dir_stack.push(&current->children.back());
      }

      vfs_interface->closedir(d);
      calc_dir_size(current);
    }
  }
  else
  {
...
bool MoveWithOverwrite(const std::string& src, const std::string& dst)
{
  ...
  int32_t size = -1;
  int flags = vfs_interface->stat(src.c_str(), &size);
...
int Seek(::retro_vfs_file_handle* handle, s64 offset, ::File::SeekOrigin origin)
{
...
int Seek(::retro_vfs_file_handle* handle, s64 offset, int origin)
{
...
u64 Tell(::retro_vfs_file_handle* handle)
{
...
int Resize(::retro_vfs_file_handle* handle, u64 size)
{
...
size_t ReadBytes(::retro_vfs_file_handle* handle, void* data, size_t length)
{
...
size_t WriteBytes(::retro_vfs_file_handle* handle, const void* data, size_t length)
{
...
int Truncate(retro_vfs_file_handle* vfs_handle, const u64 size)
{
