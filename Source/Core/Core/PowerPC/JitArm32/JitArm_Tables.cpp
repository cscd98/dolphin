...
constexpr std::array<JitArmOpTemplate, 54> s_primary_table{{
	...
	{11, &JitArm::cmpi},                  // cmpi
	{12, &JitArm::addic},                 // addic
	{13, &JitArm::addic},                 // addic_rc
...
{
	JIT_LOG("CompileInstruction: PC=%08x blockStart=%08x OPCD=%u SUBOP=%u RA=%u RB=%u RD=%u LI=%08x BD=%04x BO=%02x BI=%02x opname=%s LK=%d branchTo=0x%08x",
       op.address,
	   js.blockStart,
	    op.inst.OPCD, op.inst.SUBOP10,
       op.inst.RA, op.inst.RB, op.inst.RD,
       op.inst.LI, op.inst.BD, op.inst.BO, op.inst.BI,
       op.opinfo ? op.opinfo->opname : "(null)",
	   op.inst.LK,
	   op.branchTo);
	...
	switch (op.inst.OPCD)
	{
	  case 16: // bcx - branch conditional
        JIT_LOG("[bcx] PC=0x%08x BO=%02x BI=%02x LK=%d branchTo=0x%08x",
             op.address, op.inst.BO, op.inst.BI, op.inst.LK, op.branchTo);
        break;
      default:
		break;
	}
  if (!IsInSpace(GetWritableCodePtr()))
  {
		JIT_LOG("ERROR: Code pointer out of bounds! ptr=%p", GetWritableCodePtr());
		return;
  }

  (this->*s_dyna_op_table[op.inst.OPCD])(op.inst);

  JIT_LOG("CompileInstruction %s (did whatever that was..) %08x", op.opinfo ? op.opinfo->opname : "(null)", op.address);
