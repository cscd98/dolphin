...
namespace Input
{
constexpr std::string source = "Libretro";
static double g_gyro[8][3] = {}; // [port][X,Y,Z]
static double g_accel[8][3] = {}; // [port][X,Y,Z]
...
public:
  ...
  std::string GetSource() const override { return Libretro::Input::source; }
...
public:
  void RegisterAll()
  {
    ...
    AddInput(new ScalarInput("AccelX", &Libretro::Input::g_accel[m_port][0]));
    AddInput(new ScalarInput("AccelY", &Libretro::Input::g_accel[m_port][1]));
    AddInput(new ScalarInput("AccelZ", &Libretro::Input::g_accel[m_port][2]));
  }
