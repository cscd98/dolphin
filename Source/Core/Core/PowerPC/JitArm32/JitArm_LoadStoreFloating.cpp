...
void JitArm::stfXX(UGeckoInstruction inst)
{
	...
	if (is_immediate)
	{
		if (jo.optimizeGatherPipe && PowerPC::IsOptimizableGatherPipeWrite(imm_addr))
		{
			...
			if (flags & BackPatchInfo::FLAG_SIZE_F64)
			{
				// Convert double to u64 and call WriteToFifo64
				PUSH(2, R0, R1);
				nemit.VREV64(I_8, D0, v0);  // Byte-swap double
				VMOV(R0, R1, D0);           // Move to R0:R1
				MOV(R2, R0);                // Lower 32 bits
				MOV(R3, R1);                // Upper 32 bits

				// Combine into u64 in R0 (if needed) or call helper with R0:R1
				// For simplicity, call helper twice:
				MOV(R0, R2);
				MOVI2R(R12, reinterpret_cast<u32>(&JitArm::WriteToFifo));
				BLX(R12);
			...
			{
				// Convert float to u32 and call WriteToFifo32
				VCVT(S0, v0, 0);
			...
			js.fifoBytesSinceCheck += (flags & BackPatchInfo::FLAG_SIZE_F64) ? 8 : 4;
		}
