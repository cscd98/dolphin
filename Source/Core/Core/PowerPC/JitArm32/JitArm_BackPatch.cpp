...
bool JitArm::BackPatch(SContext* ctx)
{
    ...
    u32 value = *reinterpret_cast<u32*>(codePtr);
    ARMReg rD = INVALID_REG;
    ARMReg V1 = INVALID_REG;
    u32 flags = 0;
    ...
    // Emit a new backpatch routine starting at the current PC
    ARMXEmitter emitter(codePtr);
    const u8* routine_start = emitter.GetCodePtr();

    // Emit the routine and get the offset (in instructions) to the faulting instruction
    u32 fault_offset = EmitBackpatchRoutine(&emitter, flags, false, true, rD, V1);

    // Flush the instruction cache to ensure the CPU sees the new code
    emitter.FlushIcache();

    // Dynamically compute the address of the faulting instruction
    const u8* fault_inst_ptr = routine_start + fault_offset * 4;

    // Patch the faulting instruction if needed
    ARMXEmitter patch_emitter(const_cast<u8*>(fault_inst_ptr));

    // Example: re-emit the instruction (you may need to customize this)
    patch_emitter.LDR(rD, ARMReg::R9);

    // Update the context to point to the new routine
    ctx->CTX_PC = reinterpret_cast<u32>(routine_start);
    return true;
}
...
u32 JitArm::EmitBackpatchRoutine(ARMXEmitter* emit, u32 flags, bool fastmem, bool do_padding, ARMReg RS, ARMReg V1)
{
    ...
    const u8* fastmem_start = code_base;
    ...
    {
        const u8* slowmem_start = emit->GetCodePtr();
    ...
    if (do_padding)
    {
        const u8* code_end = emit->GetCodePtr();
        u32 code_size = static_cast<u32>(code_end - code_base) / 4;

        constexpr u32 kMaxBackpatchInstructions = 64;
        if (code_size < kMaxBackpatchInstructions)
            emit->NOP(kMaxBackpatchInstructions - code_size);
    }
...
void JitArm::InitBackpatch()
{
  ...
  auto emit_pair = [&](u32 flags, ARMReg reg1, ARMReg reg2 = INVALID_REG)
  {
    // Emit slowmem path
    EmitBackpatchRoutine(this, flags, false, false, reg1, reg2);
    code_end = GetWritableCodePtr();
    u32 slowmem_size = (code_end - code_base) / 4;

    SetCodePtr(code_base);

    // Emit fastmem path with padding to match slowmem
    EmitBackpatchRoutine(this, flags, true, true, reg1, reg2);
    code_end = GetWritableCodePtr();
    u32 fastmem_size = (code_end - code_base) / 4;

    SetCodePtr(code_base);

    // Store the info (even if empty, for lookup consistency)
    m_backpatch_info[flags] = info;
  };

  // Writes
  emit_pair(BackPatchInfo::FLAG_STORE | BackPatchInfo::FLAG_SIZE_8, R0);
  emit_pair(BackPatchInfo::FLAG_STORE | BackPatchInfo::FLAG_SIZE_16, R0);
  emit_pair(BackPatchInfo::FLAG_STORE | BackPatchInfo::FLAG_SIZE_32, R0);
  emit_pair(BackPatchInfo::FLAG_STORE | BackPatchInfo::FLAG_SIZE_F32, D0);
  emit_pair(BackPatchInfo::FLAG_STORE | BackPatchInfo::FLAG_SIZE_F64, D0);

  // Loads
  emit_pair(BackPatchInfo::FLAG_LOAD | BackPatchInfo::FLAG_SIZE_8, R0);
  emit_pair(BackPatchInfo::FLAG_LOAD | BackPatchInfo::FLAG_SIZE_16, R0);
  emit_pair(BackPatchInfo::FLAG_LOAD | BackPatchInfo::FLAG_SIZE_32, R0);
  emit_pair(BackPatchInfo::FLAG_LOAD | BackPatchInfo::FLAG_SIZE_16 | BackPatchInfo::FLAG_REVERSE, R0);
  emit_pair(BackPatchInfo::FLAG_LOAD | BackPatchInfo::FLAG_SIZE_16 | BackPatchInfo::FLAG_EXTEND, R0);
  emit_pair(BackPatchInfo::FLAG_LOAD | BackPatchInfo::FLAG_SIZE_32 | BackPatchInfo::FLAG_REVERSE, R0);
  emit_pair(BackPatchInfo::FLAG_LOAD | BackPatchInfo::FLAG_SIZE_F32, D0, D1);
  emit_pair(BackPatchInfo::FLAG_LOAD | BackPatchInfo::FLAG_SIZE_F64, D0);
}
