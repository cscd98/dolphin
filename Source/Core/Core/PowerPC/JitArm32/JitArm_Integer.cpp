...
constexpr u32 CRFD = 3;

void JitArm::ComputeRC(ARMReg value)
{
	ARMReg rB = gpr.GetReg();

	Operand2 ASRReg(value, ST_ASR, 31);

	STR(value, R9, PPCSTATE_OFF(cr.fields[0]));
	MOV(rB, ASRReg);
	STR(rB, R9, PPCSTATE_OFF(cr.fields[0]) + sizeof(u32));

	gpr.Unlock(rB);
}

void JitArm::ComputeRC3(ARMReg value)
{
	ARMReg rB = gpr.GetReg();

	Operand2 ASRReg(value, ST_ASR, 31);

	STR(value, R9, PPCSTATE_OFF(cr.fields[CRFD]));
	MOV(rB, ASRReg);
	STR(rB, R9, PPCSTATE_OFF(cr.fields[CRFD]) + sizeof(u32));

	gpr.Unlock(rB);
}

void JitArm::ComputeRC(s32 value)
{
	ARMReg rB = gpr.GetReg();
	...
	MOVI2R(rB, value);
	STR(rB, R9, PPCSTATE_OFF(cr.fields[0]));
	MOV(rB, ASRReg);
	STR(rB, R9, PPCSTATE_OFF(cr.fields[0]) + sizeof(u32));
...
void JitArm::ComputeRC3(s32 value)
{
	ARMReg rB = gpr.GetReg();
	...
	MOVI2R(rB, value);
	STR(rB, R9, PPCSTATE_OFF(cr.fields[CRFD]));
	MOV(rB, ASRReg);
	STR(rB, R9, PPCSTATE_OFF(cr.fields[CRFD]) + sizeof(u32));
...
void JitArm::cmp (UGeckoInstruction inst)
{
	...
	{
		ComputeRC3((s32)gpr.GetImm(a) - (s32)gpr.GetImm(b));
		return;
	}
...
void JitArm::cmpl(UGeckoInstruction inst)
{
	...
	if (gpr.IsImm(a) && gpr.IsImm(b))
	{
		ComputeRC3(gpr.GetImm(a) - gpr.GetImm(b));
		return;
	}
...
void JitArm::cmpi(UGeckoInstruction inst)
{
	...
	if (gpr.IsImm(a))
	{
		ComputeRC3((s32)gpr.GetImm(a) - inst.SIMM_16);
		return;
	}
	ARMReg rA = gpr.GetReg();
	ARMReg RA = gpr.R(a);
...
void JitArm::cmpli(UGeckoInstruction inst)
{
	...
	if (gpr.IsImm(a))
	{
		ComputeRC3(gpr.GetImm(a) - inst.UIMM);
		return;
	}
