...
void JitArm::bx(UGeckoInstruction inst)
{
  ...
  if (inst.LK)
  {
    ...
    LogRegFromJIT("(bx): inst.LK is set (WA holds js.compilerPC + 4): ", WA);
  }

  // Mid-block: no forced dispatch. Let the block continue.
  if (!js.isLastInstruction)
  {
    LogNumFromJIT("(bx): not last instruction");
    ...
    if (inst.LK && !js.op->skipLRStack)
    {
      ...
      {
        LogNumFromJIT("(bx): blr optimization disabled so jumping to js.compilerPC + 4", js.compilerPC + 4);
  ...
  if (IsDebuggingEnabled())
  {
    LogNumFromJIT("(bx): IsDebuggingEnabled true");
    auto wb = gpr.GetScopedReg();
    auto wc = gpr.GetScopedReg();
...
void JitArm::bclrx(UGeckoInstruction inst)
{
  ...
  {
    LogNumFromJIT("bclrx: matches either conditional || inst.LK || IsDebuggingEnabled()");
